...
register_activation_hook(__FILE__, 'bb_data_plugin_create_table');

// Create database table on plugin activation
function bb_data_plugin_create_table()
{
    global $wpdb;

    $table_name = $wpdb->prefix . 'bbdatas';
...
// Handle CSV import
add_action('wp_ajax_import_csv_data', 'bb_data_plugin_import_csv');
function bb_data_plugin_import_csv()
{
    // Verify nonce
            ...
            if (is_uploaded_file($_FILES['file']['tmp_name'])) {
                ...
                while (($line = fgetcsv($csvFile)) !== FALSE) {
                    ...
                    if (!empty($line_arr)) {
                        ...
                        // Check whether bbdata already exists in the database with the same email
                        $existing = $wpdb->get_var($wpdb->prepare("SELECT id FROM $table_name WHERE email = %s", $email));

                        if ($existing) {
                            // Update bbdata data in the database
                            $wpdb->update(
                                $table_name,
                                array(
                                    'name' => $name,
                            ...
                            // Insert bbdata data in the database
                            $wpdb->insert(
                                $table_name,
                                array(
                                    'name' => $name,
                ...
                $res_status = 'success';
                $res_msg = 'bbdatas data has been imported successfully.';
            } else {
                $res_status = 'danger';
...
add_action('admin_menu', 'bb_data_plugin_menu');
function bb_data_plugin_menu()
{
    add_menu_page(
        //page title
        'New data',
...
function bb_data_plugin_admin_page()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'bbdatas';
    ...
    <div class="wrap">
        ...
        <div class="row">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <?php
                    // Fetch bbdata records from database 
                    ...
                    if ($result) {
                        ...
                        <tr>
                            <td colspan="6" style="text-align: center; color: #666;">No bbdata(s) found...</td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function downloadSample() {
            ...
            link.setAttribute("download", "sample-bbdatas.csv");
