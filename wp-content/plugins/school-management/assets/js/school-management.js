jQuery(document).ready(function($) {
            ...
            success: function(data) {
                ...
                if (data && data.length > 0) {
                    $.each(data, function(index, school) {
                        $dropdown.append('<option value="' + school.ID + '">' + school.post_title + '</option>');
                    });
                }
            },
            error: function() {
    ...
    function loadClasses(schoolId) {
        $.ajax({
            url: schoolManagementAjax.apiUrl + 'school-management/v1/classes/' + schoolId,
            method: 'GET',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('X-WP-Nonce', schoolManagementAjax.nonce);
            },
            success: function(data) {
                ...
                if (data && data.length > 0) {
                    $.each(data, function(index, classItem) {
                        $dropdown.append('<option value="' + classItem.ID + '">' + classItem.post_title + '</option>');
                    });
                }
            },
            error: function() {
    ...
    function loadEntities(classId) {
        $.ajax({
            url: schoolManagementAjax.apiUrl + 'school-management/v1/entities/' + classId,
            method: 'GET',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('X-WP-Nonce', schoolManagementAjax.nonce);
            },
            success: function(data) {
                ...
                if (data && data.length > 0) {
                    $.each(data, function(index, entity) {
                        ...
                        if (entity.link) {
                            entityHtml = '<a href="' + entity.link + '" class="entity-item" target="_blank">';
                        }
                        
                        if (entity.image) {
                            entityHtml += '<img src="' + entity.image + '" alt="' + entity.title + '">';
                        } else {
